...
const AddShows = () => {
  ...
  const handleDateTimeAdd = () => {
    ...
    // Convert the local datetime-local value to an ISO string (UTC)
    const iso = new Date(dateTimeInput).toISOString();

    setDateTimeSelection((prev) => {
      const times = prev[date] || [];
      if (!times.includes(iso)) {
        return { ...prev, [date]: [...times, iso] };
      }
      return prev;
    });
  };
  const handleRemoveTime = (date, timeToRemove) => {
    setDateTimeSelection((prev) => {
      const updatedTimes = prev[date].filter((time) => time !== timeToRemove);

      // If no times left for this date, remove the date key
  ...
  return nowPlayingMovies.length > 0 ? (
    <>
      ...
      <button
        ...
        disabled={addingShow}
        className="bg-primary text-white px-8  py-2 mt-6 rounded hover:bg-primary/90 transition-all curson-coinder"
      >
        Add Show
      </button>
    </>
